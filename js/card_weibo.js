<link rel="stylesheet" class="aplayer-secondary-style-marker" href="\assets\css\APlayer.min.css"><script src="\assets\js\APlayer.min.js" class="aplayer-secondary-script-marker"></script><script class="meting-secondary-script-marker" src="\assets\js\Meting.min.js"></script>fetch("https://weibo-top-api.vercel.app/api")
  .then((data) => data.json())
  .then((data) => {
    let html =
    ...
    for (let item of data)
      html +=
        ...
        '">' +
        (item.hot || "荐") +
        '</div><span class="weibo-title"><a title="' +
        item.title +
        '"href="' +
        item.url +
        '" target="_blank" rel="external nofollow noreferrer">' +
        item.title +
        '</a></span><div class="weibo-num"><span>' +
        item.num +
        "</span></div></div>";
    (html += "</div>"),
